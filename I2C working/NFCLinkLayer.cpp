...
uint32_t NFCLinkLayer::openLinkToServer(boolean debug)
{
   ...
   if (debug)
   {
      Serial.println(F("Opening link to server."));
   }
  ...
  receivedPDU = ( PDU *) DataIn; 
   ...
   Serial.println("Connect PDU: ");
   Serial.println(targetPayload.field[0], HEX);
  ...
  _nfcReader->targetRxData(DataIn); // receive an CCPDU
   ...
   if (receivedPDU->getPTYPE() != CONNECTION_COMPLETE_PTYPE)
   {
      if (debug)
      {
         Serial.println(F("Connection Completely Failed."));
      }
...
uint32_t NFCLinkLayer::openLinkToClient(boolean debug) 
{
   ...
   buildCCPDU(&targetPayload, receivedPDU);
...
uint32_t NFCLinkLayer::transmitToServer(uint8_t *nppMessage, uint32_t len, boolean debug)
{
   ...
   */
    if (debug)
     {
    Serial.print(F("<transmitToServer> Minne:"));
...
void NFCLinkLayer::buildCCPDU(PDU *targetPayload, PDU *receivedPDU)
{
